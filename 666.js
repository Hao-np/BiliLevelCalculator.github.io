document.addEventListener('DOMContentLoaded', function() {
    ...
    resultButton.addEventListener('click', function() {
        ...
        // 计算每天获得的经验
        let dailyExperience = (login.classList.contains('mdui-chip-selected') ? 5 : 0) +
                              (watch.classList.contains('mdui-chip-selected') ? 5 : 0) +
                              (share.classList.contains('mdui-chip-selected') ? 5 : 0) +
                              (dailyCoins * 10);

        // 如果没有选择任何选项或输入任何值，则不进行计算
        if (dailyExperience === 0 && coin === 0) {
            mdui.snackbar({
                message: "请至少选择一个选项或输入一个值！",
                position: 'bottom',
                timeout: 3000
            });
            return;
